...
function inferScene(routeText, highlights) {
  const fromHighlights = highlights.find((item) => item.scene && item.scene !== "city")?.scene
    || highlights.find((item) => item.scene)?.scene;
...
function buildHighlights(stops) {
  return stops
    .filter((stop, index, array) => array.indexOf(stop) === index)
    .map((stop) => {
      const meta = PLACE_META[stop];
      const photoOverride = PLACE_PHOTO_OVERRIDES[stop] || null;
      if (!meta) return null;
      return {
        name: stop,
        ...
        image: photoOverride?.image || meta.image,
        coord: meta.coord,
        credit: photoOverride ? (photoOverride.credit ?? null) : (meta.creditKey ? PHOTO_CREDITS[meta.creditKey] : null)
      };
    })
    .filter(Boolean);
}
...
export function parseRoadbookText(source) {
  ...
  return lines.map((line, index) => {
    ...
    const scene = { key: sceneKey, ...SCENE_META[sceneKey] };
    const title = buildTitle(stops, routeText);
    const isRestDay = /休整|整备|徒步/.test(line) || (!distance && !hours);
    ...
    return {
      ...
      scene,
      highlights,
      statusLabel: isRestDay ? "休整" : fatigue >= 12 ? "高压日" : "推进中"
    };
  });
}
